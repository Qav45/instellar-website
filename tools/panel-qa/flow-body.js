...
(function(){
  ...
  async function run(){
    ...
    P.openPunish({type:'Ban',target:'void_runner',reason:'Cheating / Hacking',duration:'7 days'}); await sleep(50);
    ...
    P.openPunish({type:'Ban',target:'RedstoneRae',reason:'Doxing / DDoSing',duration:'Permanent'}); await sleep(50);
    click('#modal-root [data-action="submit"]'); await sleep(300);
    row=MOCK.db.mod_actions[MOCK.db.mod_actions.length-1]; log('4 perm ban status', row.status, '| toast:', toasts());
    // iron_ivy has an active in-game ban in the ledger (no mod_actions row): a second Ban must warn first.
    before=MOCK.db.mod_actions.length;
    P.openPunish({type:'Ban',target:'iron_ivy',reason:'Cheating / Hacking',duration:'7 days'}); await sleep(50);
    click('#modal-root [data-action="submit"]'); await sleep(300);
    var again=q('#modal-root [data-action="confirmReplace"]');
    log('4b already banned: rows added', MOCK.db.mod_actions.length-before, '| warning:', txt('#modal-root .p-rebans .note-line'), '| kv:', Array.prototype.map.call(document.querySelectorAll('#modal-root .pl-kv'),function(e){return e.textContent.trim();}).join(' / '), '| button:', again?again.textContent.trim():'(none)', '| form gone:', !q('#modal-root [data-action="submit"]'));
    document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true})); await sleep(50);
    log('4c escape: modal open', !!P.modal.current, '| rows added', MOCK.db.mod_actions.length-before);
    P.openPunish({type:'Ban',target:'iron_ivy',reason:'Cheating / Hacking',duration:'7 days'}); await sleep(50);
    click('#modal-root [data-action="submit"]'); await sleep(50);
    click('#modal-root [data-action="back"]'); await sleep(50);
    log('4d back: form back', !!q('#modal-root [data-action="submit"]'), '| reason kept:', q('#modal-root [data-field="reason"]')&&q('#modal-root [data-field="reason"]').value);
    click('#modal-root [data-action="submit"]'); await sleep(50);
    click('#modal-root [data-action="confirmReplace"]'); await sleep(300);
    row=MOCK.db.mod_actions[MOCK.db.mod_actions.length-1];
    log('4e confirmed: rows added', MOCK.db.mod_actions.length-before, row.type, row.target, row.status, row.duration, '| modal open:', !!P.modal.current, '| toast:', toasts());
    // a Mute on the same banned player is untouched by the guard
    before=MOCK.db.mod_actions.length;
    P.openPunish({type:'Mute',target:'iron_ivy',reason:'Spam',duration:'1 hour'}); await sleep(50);
    click('#modal-root [data-action="submit"]'); await sleep(300);
    log('4f mute on banned player: rows added', MOCK.db.mod_actions.length-before, '| modal open:', !!P.modal.current);
    ...
    click('[data-action="range"][data-v="30"]'); await sleep(100); var rc=q('[data-action="range"][data-v="30"]'); log('10 range 30 active:', rc?rc.classList.contains('is-active'):'(no chip)');
    ...
    // Wipeban gets the same guard (Supervisor has the permission)
    before=MOCK.db.mod_actions.length;
    P.openPunish({type:'Wipeban',target:'iron_ivy',reason:'Doxing / DDoSing'}); await sleep(50);
    click('#modal-root [data-action="submit"]'); await sleep(300);
    again=q('#modal-root [data-action="confirmReplace"]');
    log('11b wipeban already banned: rows added', MOCK.db.mod_actions.length-before, '| button:', again?again.textContent.trim():'(none)');
    click('#modal-root [data-action="confirmReplace"]'); await sleep(300);
    row=MOCK.db.mod_actions[MOCK.db.mod_actions.length-1];
    log('11c wipeban confirmed: rows added', MOCK.db.mod_actions.length-before, row.type, row.target, row.status, row.duration, '| modal open:', !!P.modal.current);
