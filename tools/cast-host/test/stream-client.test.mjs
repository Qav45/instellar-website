...
// goes to the decoder, what the state line says when it all falls over, and
// which codecs to ask the host for once the browser has said what it decodes.
...
const { LAG_QUEUE, STREAM_CODECS, parseVideoFrame, streamGate, streamReason, streamCodecs } = vm.runInContext(
  ...
  grab("function streamCodecs(supported) {", "\n}\n") +
  "({ LAG_QUEUE, STREAM_CODECS, parseVideoFrame, streamGate, streamReason, streamCodecs });",
  sandbox);
